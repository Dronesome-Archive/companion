...
class KCFTracker(object):
    ...
    def construct_hanning_materials(self):
    ...
    def construct_gaussian_peak(self, size_y, size_x):
        syh, sxh = size_y / 2, size_x / 2
        output_sigma = np.sqrt(size_x * size_y) / self.padding * self.output_sigma_factor
    ...
    def get_features(self, image, init_hann, scale_adjust=1.0):
        ...
        if self._hogfeatures:
            mapp = dict({'size_x': 0, 'size_y': 0, 'feature_count': 0, 'feature_map': 0})
            mapp = get_feature_maps(z, self.cell_size, mapp)
            mapp = prepare(mapp, 0.2)
            mapp = get_PCA_feature_maps(mapp)
            self.size_patch = [mapp['size_y'], mapp['size_x'], mapp['feature_count']]
            feature_map = mapp['feature_map'].reshape((self.size_patch[0] * self.size_patch[1], self.size_patch[2])).T 
        else:
            if z.ndim == 3 and z.shape[2] == 3:
                feature_map = cv2.cvtColor(z, cv2.COLOR_BGR2GRAY) 
            elif z.ndim == 2:
                feature_map = z   
            feature_map = feature_map.astype(np.float32) / 255.0 - 0.5
            self.size_patch = [z.shape[0], z.shape[1], 1]

        if init_hann:
            self.construct_hanning_materials()

        return self.hann * feature_map
    ...
    def forward(self, x, train_interp_factor):
    ...
    def init(self, roi, image):
        ...
        self._prob = self.construct_gaussian_peak(self.size_patch[0], self.size_patch[1])
        self._alphaf = np.zeros((self.size_patch[0], self.size_patch[1], 2), np.float32)
        
        self.forward(self._tmpl, 1.0)
    ...
    def update(self, image):
        ...
        self.forward(x, self.interp_factor)
